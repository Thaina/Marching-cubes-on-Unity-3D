...
public class ChunkManager : Singleton<ChunkManager>
{
	...
	/// </summary>
	public void AddSnow(float3 modificationPoint,int underCloudHeight, float range, float modification)
	{
		...
		int intRange = (int)(range * Constants.VOXEL_SIDE);//range /2 because the for is from -intRange to +intRange
		...
		for (int z = -intRange; z <= intRange; z++)
		{
			for (int x = -intRange; x <= intRange; x++)
			{
				...
				var vertexPoint = vertexOrigin + new float3(x,0,z);
				//Avoid edit the first and last height vertex of the chunk, for avoid non-faces in that heights
				float distance = math.distance(vertexPoint.xz, modificationPoint.xz) / range;
				if(distance > 1)//Not in range of modification, we check other vertexs
				{
					//Debug.Log("no Rango: "+ distance + " > " + range+ " |  "+ vertexPoint +" / " + modificationPoint);
					continue;
				}

				int chunkModification = (int)(modification * (1 - distance));
				if(chunkModification == 0)
					continue;

				foreach(var (chunk,point) in GetAroundChunk(vertexPoint))
				{
					var vertexChunk = point;
					vertexChunk.y = underCloudHeight;

					int value = 0;
					int material = 0;
					while(vertexChunk.y > 0)
					{
						vertexChunk.y--;
						(value,material) = chunk.GetData(vertexChunk);
		...
		for (int y = -intRange; y <= intRange; y++)
		{
			for (int z = -intRange; z <= intRange; z++)
			{
				for (int x = -intRange; x <= intRange; x++)
				{
					...
					float distance = math.distance(vertexPoint, modificationPoint) / range;
					if(distance > 1)//Not in range of modification, we check other vertexs
					{
						//Debug.Log("no Rango: "+ distance + " > " + range+ " |  "+ vertexPoint +" / " + modificationPoint);
						continue;
					...
					int chunkModification = (int)(modification * (1 - distance));
					foreach(var (chunk,point) in GetAroundChunk(vertexPoint))
						chunk.modifyTerrain(point,chunkModification,mat);
				}
			}
		}
	}

	IEnumerable<(Chunk chunk,int3 point)> GetAroundChunk(float3 vertexPoint)
	{
		...
		if(chunkDict.TryGetValue(hitChunk,out var chunk00))
			yield return (chunk00,vertexChunk);
		...
		if(isZero.x && chunkDict.TryGetValue(hitChunk - new int2(1,0),out var chunk10))//Interact with vertex of chunk(-1,0)
			yield return (chunk10,new int3(vertexChunk) { x = Constants.CHUNK_SIZE });
		
		if(isZero.y && chunkDict.TryGetValue(hitChunk - new int2(0,1),out var chunk01))//Interact with vertex of chunk(0,-1)
			yield return (chunk01,new int3(vertexChunk) { z = Constants.CHUNK_SIZE });
		...
		if(math.all(isZero) && chunkDict.TryGetValue(hitChunk - new int2(1,1),out var chunk11))//Interact with chunk(-1,-1)
			yield return (chunk11,new int3(vertexChunk) { xz = Constants.CHUNK_SIZE });
	}
